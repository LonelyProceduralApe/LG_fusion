...
def train():
    # output dir
    output_dir = 'new_' + HyperParams['kind'] + '_' + HyperParams['arch'] + '_output'
    try:
        os.stat(output_dir)
    except:
    ...
    # trainset = torchvision.datasets.ImageFolder(root=r'/home/hello/twelve/ablation/redata/train', transform=transform_train)  # 训练数据集
    trainset = torchvision.datasets.ImageFolder(root=r'/home/cver/zjc/Ours/cub/train', transform=transform_train)  # 训练数据集
    # trainset = torchvision.datasets.ImageFolder(root=r'/home/hello/twelve/imgs_data/cutcar/car_train', transform=transform_train)  # 训练数据集
    # trainset = torchvision.datasets.ImageFolder(root=r'/home/cver/zjc/Ours/fgvc-aircraft-2013b/data/dataset/train', transform=transform_train)  # 训练数据集
    # print(trainset)
    trainloader = DataLoader(trainset, batch_size=BATCH_SIZE, shuffle=True)  # 生成一个个batch进行批训练，组成batch的时候顺序打乱取

    # testset = torchvision.datasets.ImageFolder(root=r'/home/hello/twelve/ablation/redata/test', transform=transform_test)
    testset = torchvision.datasets.ImageFolder(root=r'/home/cver/zjc/Ours/cub/test', transform=transform_test)
    # testset = torchvision.datasets.ImageFolder(root=r'/home/hello/twelve/imgs_data/cutcar/car_test', transform=transform_test)
    # testset = torchvision.datasets.ImageFolder(root=r'/home/cver/zjc/Ours/fgvc-aircraft-2013b/data/dataset/test', transform=transform_test)
    ...
    for epoch in range(0, HyperParams['epoch']):
        ...
        # eval
        val_acc = o_test_net(net, testloader)
